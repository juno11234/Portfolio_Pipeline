 ...
 *
 * 산문 서술 규칙(2026-08-04 사용자 결정): lede·intro·code notes.why·decision reason·
 * callout.body·회고 body 등 '읽는 산문'에서는 클래스·함수·필드명 남발을 피하고 자연어로
 * 풀어쓴다(예: `Sequence_Memory` → '진행 위치를 기억하는 노드'). 자료구조·파라미터·기술명
 * (Stack·DAG·UniTask 등)은 남겨도 된다. 예외: 코드 블록(code.code)·다이어그램(mermaid.source)·
 * 표(table·stack-table)·계층 노드(layers.items)·api-list 시그니처는 식별자를 그대로 쓴다 —
 * 거기선 이름이 곧 정보다. Zod 로 자동 검증하지 않는다(정당한 기술명까지 오탐 → 게이트 왜곡).
 * 새 문서를 조립할 때 이 규칙을 지킨다.
 * ========================================================================== */
