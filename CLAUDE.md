...
- 디자인 섹션 01 의 **기술 스택 표는 `architecture.parts[0]` 로 흡수**(overview 엔 blocks 가 없다 —
  뱀서 관례). 디자인 5계층 아키텍처 → `layers`, 세 인터페이스 계약 → 하나의 mermaid classDiagram.
- 디자인 섹션 07(데이터·빌드·회고)은 **`features[데이터·빌드]` + `retrospective` 로 쪼갰다** — 데이터
  파이프라인·Jenkins 는 ③ 핵심 기능이지 회고가 아니다. 회고는 parts[만족] + debt(아쉬웠던 점) + plan(다음 계획).
