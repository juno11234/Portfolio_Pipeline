...
> **표의 세 문서(뱀서·Bond·SoulLike) 다 이미 코드 유래로 넘어갔다**(뱀서·Bond 2026-07-18 · SoulLike 2026-07-20, 위 "지금 상태"). PDF 는 더 이상 출처가 아니다 — **이제 PDF 유래 문서는 없다**(나중에 붙은 기업협약은 처음부터 코드 유래). 이 표는 옛 출처의 기록으로만 남긴다.
...
**뱀서라이크가 코드 유래 신 파이프라인 산출물로 교체됐고, 사이트가 GitHub Pages 로 배포됐다**(2026-07-18).
PDF 에서 옮긴 옛 `vampire-like.yaml` 을 걷어내고, 대상 프로젝트의 소스 코드 → 3양식 md(`1-구현`·`2-판단`·`3-자산`,
`src/assets/vampire-like/` 에 커밋) → YAML 로 다시 세웠다 — 파이프라인의 존재 이유가 실제로 한 번 돌았다.
그때 **카드 레이아웃 자동 규칙**(가로↔세로, 본문 길이)·**VS 다크 코드**·전폭 세로 카드(`StackCards`)·
텍스처 전후 `figure-grid`·깨달음 `LessonCard`(cells) 도 함께 들어갔다(위 "확정된 결정").
라이브: **https://juno11234.github.io/Portfolio_Pipeline/** (여섯 문서 전부 코드 유래 — PDF 유래는 없다).
> ⚠ 아래 이력 서술 중 일부는 이 교체 이전 기준이다 — 특히 `vampire-like.yaml` 을 "PDF 유래 완성본"으로 부르는 곳.

**그 뒤 Bond·기업협약·SoulLike 도 코드 유래로 넘어갔다**(Bond 2026-07-18 · 기업협약·SoulLike 2026-07-20, git 이력).
이로써 **문서 넷(뱀서·Bond·SoulLike·기업협약)이 전부 코드 유래**다 — `docs/*.yaml` 헤더가 "코드→md→YAML"로 조립됐음을
밝힌다. **PDF 유래 문서는 이제 없다.** (기업협약 소스 md 는 폴더명이 `src/assets/Corporate/`.)

SoulLike 재작성(2026-07-20) 때 플레이어 **FSM 은 mermaid 상태도 대신 실제 Animator 스크린샷**으로 냈다 — 허브가
많은 상태 그래프는 mermaid 자동배치로 안 읽혀 사용자가 이미지를 택했다(Unity Animator 창이 곧 상태 머신이라 중복도
없앴다). "그래프=mermaid" 원칙의 첫 예외다. 겸사겸사 잠재 버그 둘도 고쳤다 — 회고 렌더러가 cells·body 를 섞으면
cells 를 통째로 버리던 것(`[slug].astro` `every`→`some`), Figure 캡션이 `display:flex` 안에서 강조를 조각내던 것
(`span` 래핑). 유튜브·깃허브 링크는 확인돼 들어갔고, `BossBT`·`MVCvsMVVM` png 는 커밋됐으나 미사용(mermaid·결정표로 대체).

**이어서 스타워즈(3d-Starwars)도 코드 유래로 조립됐다**(2026-07-21) — 다섯 번째 문서. 벡터 수학 파쿠르(외적·투영)·
이벤트 큐 전투·옵저버 QTE·이원화 AI(적 enum FSM + 보스 State 패턴 FSM)를 담았고, mermaid 6개(라벨 겹침 0)·보스 스킬
2샷 `figure-grid`. 코드의 실제 오타 식별자(`CombatSysytem`·`EvenetType`·`Reciever`·`ExcutionReady`)는 지어내지 않고
그대로 뒀다(yaml 헤더에 표시). 슬러그는 `starwars-jedi`(자산 폴더는 `starwars`).

**버민타이드(3D-Vermintide)로 여섯 문서가 다 찼다**(2026-07-21) — **프로젝트 6개 전부 기술 문서가 있고, 전부 코드 유래다.**
이벤트 큐 전투·전략 패턴 무기·상속 몬스터(4종)·상태 패턴 보스·오브젝트 풀·웨이브 스폰을 10섹션에 담았다
(mermaid 9개 라벨 겹침 0 · 코드 15블록 · 스크린샷 7장). **스키마도 dc 도 한 줄 안 고쳤다** — 여섯 번째 문서인데
새 블록·새 prop 이 0이다(게이트가 굳었다는 가장 강한 증거).

이번엔 **① 이 인용한 수치·코드를 대상 저장소에서 전부 재확인**했다(파이프라인에서 처음). 전부 일치했고, 덤으로 ② 부록의
확인 요청 둘이 풀렸다 — 점프 공식은 실제로 `Mathf.Sqrt(jumpHeight * -1f * gravity)`(2 곱 없음), HashSet 중복 필터는
대검 **스킬** 콜라이더(`GreatSwordSkill`)에 있고 `OnEnable` 에서 `Clear` 하는 것도 ② 서술대로였다.
**① 을 코드와 대조하는 이 단계를 앞으로도 넣는다.** 조립 비용이 거의 안 들면서 지어낸 수치를 막는다.
그 밖의 판단: 기간은 `2025.04.14 ~ 05.02`(git 첫 커밋~README 종료일, 사용자 결정) · `SavingSystem` 스텁은
**안 넣었다**(구현된 시스템만 다룬다) · 장르 표기는 `projects.yaml` 의 `1인칭 슈팅` 에 맞췄다.
...
개요   Pills  Figure  WhatWhy  GoalCards  OverviewLede  Tldr(안 씀)  StackTable
...
> **`StackCards`(카드 1열 세로)는 이 목록에 없다** — 뜬(byte-0) 게 아니라 `PrincipleCards` 카드를 1열로 세운
> authored 변형이다(카드 style 문자열은 그대로 재사용). 위 "확정된 결정 — 카드 레이아웃" 참조.
> `Code`(코드 블록)는 색칠을 Shiki(VS 다크)로 바꿨다 — 프레임만 byte-0, 색은 grammar 하이라이팅.
> **`Tldr`(▶ 10초 요약)는 파일만 남고 아무 데서도 안 그린다** — 2026-07-22 개요에서 뺐다(아래 "확정된 결정").
...
- **추가 블록:** `api-list`(시그니처+반환 필수 — 규칙) · `callout`(곁가지) ·
  `chips`(단방향 인라인 상태/흐름; 분기 생기면 mermaid) · `figure-grid`(그림 2장 나란히 — 텍스처 전후 등, 07-18 추가).
- **variant 필드:** `points`(plain·numbered·check·mini·flow·defect·**stack** + columns 2/3) ·
  `table`(plain·weight·channel) · `image`(below·aside + width). 모양만 다르고 데이터는 하나.
  (`points` 가로↔세로는 이제 렌더러가 본문 길이로 자동 결정 — `stack` 은 강제 세로 override. 위 "확정된 결정".)
- **retroCard 확장(07-18):** `cells`(의문점·원인·알게된 점 3칸 → 전폭 세로 `LessonCard`) · `tint`. 없으면 단순 {제목,내용} 카드.
...
ChapterDivider·PartTitle·DocFooter)와 개요 조각(Pills·Figure·WhatWhy/OverviewLede/GoalCards)을
dc 로 그리고 본문은 DocBlock 에 넘긴다. (개요 끝에 `Tldr` 도 있었으나 2026-07-22 뺐다 — 아래 "확정된 결정".)
...
1. **여백을 렌더러 규칙으로 완전히 옮긴다** — `[slug]`/`DocBlock` 이 이미 블록 종류·위치별로 주지만
   ...
   통일하는 방식을 사용자와 정한다.
2. ~~나머지 프로젝트 기술 문서~~ — **여섯 개 전부 끝났다**(마지막 버민타이드 2026-07-21). 남은 일은 1번뿐이다.

**끝난 것 (2026-07-20 세션):** `soulslike.yaml` 작성·검증(마커 0-miss, mermaid 6개) · 정적 페이지 셋·cmp 둘 삭제로
...
- **그래프 도식은 mermaid 다.** 반복되는 것(Layers 는 규칙 있어 dc 유지)만 예외. 손그림 그래프
  ...
  **또 한 예외: SoulLike 플레이어 FSM 은 실제 Animator 스크린샷**(2026-07-20, 사용자 결정) — 허브가 많은
  상태 그래프는 mermaid 자동배치로 안 읽히고, Unity Animator 창이 곧 상태 머신이라 중복이라서. 대상 프로젝트가
  이미 만든 상태도 이미지가 있으면 그걸 쓴다.
...
- **칸보다 작은 그림은 `figure-grid` 에서 픽셀을 박는다** (2026-07-21, 버민타이드 몬스터 4장에서 나온 규칙).
  폭 100%(기본)는 원본이 칸(≈421px)보다 작으면 **확대해 뭉개고**, 두 칸의 높이가 어긋나면 짧은 쪽 아래에
  **빈 배경이 남는다** — `Wave.png`(193px)가 2.18배로 늘고 옆 칸엔 325px 빈칸이 생겼다.
  **높이를 하나로 통일하고 비율대로 폭을 구해 `width`/`height` 를 준다**(전부 축소만 · 두 칸 높이 일치).
  원본이 칸보다 크면 그냥 폭 100%. 픽셀 고정은 반응형이 아니므로 **칸 폭을 넘지 않게** 잡는다(모바일에서 가로 스크롤).
  남는 여백은 **`DocBlock` 이 `margin-inline:auto` 로 가운데 정렬**한다(2026-07-21, 사용자 요청) — 왼쪽에 몰리면
  덜 만든 것처럼 보이고 가운데면 매트로 읽힌다. dc 의 `FigureGrid` 는 byte-0 라 안 건드리고 **어댑터에서** 준다
  (칩 `tone→kind` 때와 같은 규칙). 폭 100% 인 그림엔 영향이 없다. 기업협약의 스킬 2샷도 같이 가운데로 왔다.
...
- **개요의 '▶ 10초 요약' 상자는 뺐다** (2026-07-22, 사용자 결정). 여섯 문서 전부에서 사라졌다.
  **뺀 것은 렌더뿐이다** — `overview.evidence` 는 스키마 **필수 게이트 그대로**이고 YAML 본문(실측 수치·버그)도
...
(`Tldr` 는 2026-07-22 개요에서 빠졌다 — 쪼갠 이유와는 무관하다).
...
- ~~나머지 프로젝트 기술 문서~~ **(해결)** — 프로젝트 6개 전부 코드 유래 문서가 섰다(마지막 버민타이드 2026-07-21).
- **뱀서라이크 소스 md 가 이 repo 에 산다** — `src/assets/vampire-like/` 의 `1-구현`·`2-판단`·`3-자산`.
  ⚠ `2-판단-직접.md` 의 improvements 가 아직 테스트 문구라, 현재 yaml 은 옛 문서 `26_0717_TechDoc` 의 실제 개선
  5건을 이어 쓴다(yaml 주석 표시). ② 를 채우면 교체할 것. md↔yaml 이 두 곳에 사는 문제는 여전 — 문서가 쌓이면 md 에서 뽑는 스크립트를 만든다.
- **배포됨 (해결)** — GitHub Pages 로 `main` push 마다 자동 배포. `astro.config.mjs` 에 `site`/`base` 지정,
  CI(`.github/workflows/deploy.yml`)가 빌드·배포한다(Playwright chromium 설치 포함 — Mermaid 빌드용).
