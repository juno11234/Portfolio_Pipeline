...
라이브: **https://juno11234.github.io/Portfolio_Pipeline/** (네 문서 전부 코드 유래 — PDF 유래는 없다).
...
1. **여백을 렌더러 규칙으로 완전히 옮긴다** — `[slug]`/`DocBlock` 이 이미 블록 종류·위치별로 주지만
   ...
   통일하는 방식을 사용자와 정한다.
2. **나머지 2개 프로젝트**(스타워즈 · 버민타이드) — 기술 문서가 아직 없다. (기업협약은 2026-07-20 코드 유래로 완료)
...
- **나머지 2개 프로젝트** (스타워즈 · 버민타이드) — 기술 문서가 없다. (뱀서·Bond·소울라이크·기업협약은 완료)
