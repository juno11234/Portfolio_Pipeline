...
라이브: **https://juno11234.github.io/Portfolio_Pipeline/** (다섯 문서 전부 코드 유래 — PDF 유래는 없다).
...
그대로 뒀다(yaml 헤더에 표시). 슬러그는 `starwars-jedi`(자산 폴더는 `starwars`). **이제 남은 건 버민타이드 하나.**
...
1. **여백을 렌더러 규칙으로 완전히 옮긴다** — `[slug]`/`DocBlock` 이 이미 블록 종류·위치별로 주지만
   ...
   통일하는 방식을 사용자와 정한다.
2. **나머지 1개 프로젝트**(버민타이드) — 기술 문서가 아직 없다. (기업협약 2026-07-20·스타워즈 2026-07-21 코드 유래로 완료)
...
- **나머지 1개 프로젝트** (버민타이드) — 기술 문서가 없다. (뱀서·Bond·소울라이크·기업협약·스타워즈는 완료)
